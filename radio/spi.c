 ...
 *
 *  Functions for using the ATMEGA16 as an SPI master.
 */
...
void SPI_Init()
{
    ...
    SPI_DDR &= ~(_BV(DD_MOSI)|_BV(DD_MISO)|_BV(DD_SS)|_BV(DD_SCK));
    // Define the following pins as output
    SPI_DDR |= (_BV(DD_MOSI) | _BV(DD_SS) | _BV(DD_SCK));
